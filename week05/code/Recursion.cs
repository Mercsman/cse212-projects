...
public static class Recursion
{
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // ADD CODE HERE
        currPath.Add(new System.ValueTuple<int, int>(x, y));

        if (maze.IsEnd(x, y)) {
            string pathAsString = "<List>{" + string.Join(", ", currPath.Select(t => $"({t.Item1}, {t.Item2})")) + "}";
            results.Add(pathAsString);
            return;
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (maze.IsValidMove(currPath, newX, newY)) {
                SolveMaze(results, maze, newX, newY, new List<System.ValueTuple<int, int>>(currPath));
            }
        }
